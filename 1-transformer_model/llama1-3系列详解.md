...
在公式中，首先进行门控计算：\sigma(xW+b)，这里使用Sigmoid函数将中间向量映射到0到1之间的范围，表示每个元素被保留的概率。然后，将输入向量x进行线性变换后与门控后的向量进行逐元素相乘，得到最终的输出向量。

GLU的引入能够帮助网络捕捉长期依赖关系：

- 门控机制：GLU激活函数通过门控机制对输入进行筛选和调控。门控部分使用Sigmoid函数对中间向量进行处理，生成一个0到1之间的门控向量。这个门控向量决定了输入的哪些部分应该被保留、哪些部分应该被抑制。通过这个机制，GLU能够有选择地过滤输入的某些部分，将重要的信息传递给下一层，而抑制不重要的信息。这种筛选机制使得网络能够更加关注长期依赖关系中的重要部分，减少对无关信息的干扰，从而更好地捕捉长期依赖关系。

* 上下文建模：GLU激活函数的门控机制是根据输入的上下文和语境来进行决策的。在序列数据中，长期依赖关系通常表现为远距离的依赖关系，需要网络能够在较长的时间步长上进行信息传递和建模。通过门控机制，GLU可以根据上下文的信息动态地调整门控向量的值，以适应不同时间步长上的长期依赖关系。这样，网络可以在处理序列数据时更好地捕捉到长期依赖关系，避免梯度消失或梯度爆炸等问题。

使用其他激活函数定义 **GLU 变体**，如下所示:
...
生成式模型的推理过程很有特点，推理生成  tokens 的过程是迭代式的。简单来说就是，用户给一个输入文本，模型会输出一个回答（长度为 N），但该过程中实际执行了 N 次模型前向传播过程。即 LLM 类模型一次推理只输出一个 token，当前轮输出的 token 与之前输入 tokens 拼接，并作为下一轮的输入 tokens ，反复多次直到遇到终止符 EOS 或生成的 token 数目达到设置的 max_new_token 才会停止。

从生成的过程中可以看出第 i 轮输入数据只比第 i+1 轮输入数据新增了一个 token，其他全部相同！因此第 i+1 轮推理时必然包含了第 i 轮的部分计算。KV Cache 优化的起因就在这里，缓存当前轮可重复利用的计算结果，下一轮计算时直接读取缓存结果，原理很简单，本质就是用空间换时间。

单论对话的 kv cache 优化是应用在 decode 推理阶段过程中的，下图很好的展示了不使用 kv cache 和使用 kv cache 的区别

![kv_opt](../images/llama/kv_opt.png)
...
上面假设 K 和 V 能直接存在缓存中，模型规模小还好，一旦模型规模很大长度很长时，KV 根本就存不进缓存。比如 Llama 7B 模型，hidden size 是 4096，假设是半精度浮点数据float16，一个Transformer Block中就有 4096* 2 *2 = 16KB的参数缓存，而Llama 2一共32个Transformer Block，所以整个模型需要16 * 32 = 512KB的缓存空间，1024 长度那就要 512MB. 而现在英伟达最好的卡 H100 的 SRAM 缓存大概是 50MB，而 A100 则是 40MB. 而 7B 模型都这样，175B 模型就更不用说了。

做大SRAM 缓存或许是一个解决方法，但是由于SRAM 缓存太贵，这条路目前也不可行。

于是退一步，放不进缓存可以放 DRAM 上去，而 DRAM 内存也就是我们常说的 GPU 显存。但 DRAM 读取到计算芯片和 SRAM 到计算芯片的速度，差了一个量级的，这会让计算芯片一直在等待。由此便会导致一个问题: Memory Wall（内存墙）。所谓内存墙简单点说就是处理器ALU计算太快，但是内存读写速度慢跟不上，导致计算单元的利用率不高。
...
* 软件层面上的话，最近的很多优化，比如 Flash Attention，Paged Attention 都可以算。Flash Attention 就是减少了计算 Softmax 时从 DRAM 内存读取数据次数，从而提高了效率。MQA 和 GQA 都属于软件层面上翻墙的一个方法。MQ z z z z
